...
export default function TripList({ items, setItems }) {
  const deleteItem = (index) => {
   ...
   const handleCheckboxChange = (index) => (event) => {
     const tempItems = [...items];
     tempItems[index].status = event.target.checked; // Update the status based on the checkbox
  ...
  return (
    <>
      {items.length > 0 ? (
        <div className="d-flex overflow-auto bg-main gap-4 py-4 my-4 w-100 m-auto justify-content-center">
          {items.map((item, index) => (
            <div className="d-flex gap-2 justify-content-center align-items-center p-2 flex-nowrap">
              <input
                type="checkbox"
                id={item.name}
                name={item.name}
              ...
              <p
                className={
                  item.status ? "text-decoration-line-through m-0" : "m-0"
                }
              >
